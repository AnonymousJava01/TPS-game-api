...
local Options = Fluent.Options
    ...
    Tabs.Exclusives:AddButton({
            Title = "Hope Old Server",
            Description = "",
            Callback = function()
                -- Fisch Old Server Finder --
local httprequest = http_request or request
local PlaceId = game.PlaceId
local HttpService = game:GetService("HttpService")
local servers = {}
local req = httprequest({Url = string.format("https://games.roblox.com/v1/games/%d/servers/Public?sortOrder=Desc&limit=100&excludeFullGames=true", 16732694052)})
...
for i=1, #servers do
    pcall(function()
        game:GetService("TeleportService"):TeleportToPlaceInstance(16732694052, servers[i], game.Players.LocalPlayer)
    end)
end
            end
        })
